...
horarioCtrl.updateHorario = async (req, res) => {
  try {
    ...
    const prueba = await Horario.findOneAndUpdate({ _id: req.params.id }, 
      { $set: {
      horario,
      activo,
      regenerar,
...
horarioCtrl.solicitudHorario = async (req, res) => {
  ...
  if (solicita == "Turno"){
    ...
    console.log('------------------', objHorario.horario);
    ...
    const agendoElDiaAnterior = await granDemanda(req.params.id, objHorario, dia, indice, autor1, autor2, autor3, autor4) //verifica a todos los autores
    ...
    if(empresa[0].aleatorio == true)
    {
      const nuevoTurno = new Turno({
        ...
        horaSolicitud,
        aleatorio: true,
        agendoAyer: agendoElDiaAnterior? true: false,
      });
