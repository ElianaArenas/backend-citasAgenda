...
interFunc.renovarHorarios = async (dia, hora) => {
    try {
        ...
        for(h=0;h<horarios.length;h++){ //itera horarios que tengan el regenerar como true
            ...
            for (i=0; i<indices; i++) { //itera indices (horas)
                for(d=0; d<7; d++){ //itera dias
                    ...
                    if (diaSeleccionado != null) {
                        const Hoy = new Date(esqHorario.horario[i].dia[d].fecha)
                        // console.log("Hoy")
                        // console.log(Hoy)
                        var fechaNueva = new Date(Hoy.setDate(Hoy.getDate() + 7)).toLocaleDateString('en-US')
            ...
            const nuevoEsquema = await RenovarHorario.findOneAndUpdate({ idHorario: idHor }, { $set: { idHorario: idNuevo, horario: esqHorario.horario } });
...
interFunc.granDemanda = async (idHor, objHorario, dia, indice, autor1, autor2, autor3, autor4) => {
...
const generateTurno = async (filter) => {
    const horarios = await Horario.find(); //Horarios
    console.log({horarios});
    for (i=0; i<=horarios.length; i++) { //itera horarios
                ...
                if (diaSeleccionado != null) {
                    ...
                    if(autor1 != null){
                        ...
                        if(profesor != null) {
                            ...
                            const clases = await Turno.find({idHorario: idHor, dia: nombreDia, indice: j, ...filter});      
                            console.log(clases)
                            if(clases.length>0) {
                            ...
                            const turnos = await Turno.find({idHorario: idHor, dia: nombreDia, indice: j,  ...filter});      
                            console.log(turnos)
                            if(turnos.length>0) {
